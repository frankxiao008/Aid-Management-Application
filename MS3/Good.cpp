...
namespace aid {
	...
	void Good::name(const char * pname) {
		...
		delete[] p;
		if (pname != nullptr) {
			int n = strlen(pname) + 1;
			good_name = new char[n];
	...
	std::fstream& Good::store(std::fstream& file, bool newline) const {
		...
		file << type<<","<< good_sku << "," << good_unit << "," << good_name << "," << num_onhand << "," << num_needed << "," << good_price << "," << taxalbe << "," << good_error;
		if (newline == true)
			file << endl;
		return file;
	}
	...
	std::fstream& Good::load(std::fstream& file) {
		...
		if (file.is_open()) {
			...
			file >> temp.type >> temp.good_sku >> temp.good_unit >> temp.num_onhand >> good_name;
			file >> temp.num_onhand >> temp.num_needed >> temp.good_price >> temp.taxalbe;
			*this = temp;
		}
	...
	std::ostream& Good::write(std::ostream& os, bool linear) const {
		...
		if (linear) {
			...
			if (taxalbe)
				...
				os << "N/A" << endl;
			os << "Quantity on hand: " << num_onhand << endl;
			os << "Quantity needed: " << num_needed << endl;
		}
		return os;
	}
	...
	std::istream& Good::read(std::istream& is) {
		...
		if (temp_taxed != 'y' && temp_taxed != 'Y'&& temp_taxed != 'n' && temp_taxed != 'N') {
			...
			temp_errorstate.message("Only (Y)es or (N)o are acceptable");
		}
		else {
		...
		if (!is.fail()) {
			...
			is >> temp_price;
			if (is.fail())
				temp_errorstate.message("Invalid Price Entry");
		}
		...
		if (!is.fail()) {
			...
			is >> temp_num_onhand;
			if (is.fail())
				temp_errorstate.message("Invalid Quantity Entry");
		}
		...
		is >> temp_num_needed;
		if (is.fail())
			temp_errorstate.message("Invalidd Quantity Needed Entry");
